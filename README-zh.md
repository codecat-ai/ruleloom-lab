...
使用 Gallery examples 可应用一组完整的精选设置。每张卡片会更新规则、宽度、代数、种子模式、种子值、边界模式，以及在提供时更新比较规则，并把可见播放重置到第一行，便于从头观察图案展开。
...
Ruleloom Lab 使用 Vitest 编写面向行为的测试，覆盖规则解码、固定和环绕边界生成、确定性种子、精选图库元数据与应用行为、规则比较汇总、URL 查询辅助函数、预设解释、键盘快捷键、纯文本导出、SVG 导出、类 RLE 导出与导入、PNG 导出渲染与文件名、剪贴板/下载行为和渲染出的 HTML 结构。
...
- 为图库添加按种子类型、边界模式和课堂难度筛选的能力。
- 添加更多面向课堂的规则、图库示例和边界模式比较演练。
