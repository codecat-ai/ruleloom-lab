...
- 提供复制会话摘要操作，可生成确定性的纯文本引导师材料，汇总当前设置、比较结果、选中的预设/图库/课程上下文、提示、代数标注和导出交接提示。
- 提供复制工作坊检查清单操作，可生成确定性的可复用执行表，把当前规则、选中的预设或课程上下文、比较重点、引导备注、计时提示、检查步骤和反思问题组合成一个本地材料。
...
- 为 Lesson path 提供可选的本地节奏预设，覆盖 Bell-ringer、Standard 和 Workshop 引导场景，并包含确定性的分钟范围、阶段标签、简洁提示，以及可复制的纯文本节奏指南。
...
- 在 `src/pacingPresets.ts` 中导出纯确定性的课程节奏预设元数据、应用和格式化辅助函数。
- 在 `src/generationAnnotations.ts` 中导出纯本地代数标注辅助函数。
- 在 `src/comparisonSets.ts` 中导出纯本地比较集合校验、存储、导入/导出和应用辅助函数。
- 在 `src/sessionSummaries.ts` 中导出纯确定性的会话摘要格式化器。
- 在 `src/workshopChecklists.ts` 中导出纯确定性的工作坊检查清单执行表格式化器。
...
使用 Lesson paths 上方的 Pacing preset 控制项，可以在 12 分钟 Bell-ringer、24 分钟 Standard 路径和 45 分钟 Workshop 探索之间切换。屏幕上可见的计时范围和 Copy timing cues 复制出的材料会使用同一个预设，因此本地交接说明会与引导师看到的内容一致。
...
使用 Copy session summary 可在不生成打印页的情况下复制轻量引导师交接材料。摘要会包含当前可见规则设置、比较结果、可用的预设/图库/课程上下文、讨论提示、本地代数标注，以及 PNG、SVG、纯文本、RLE 和教师说明导出的提醒。

使用 Copy workshop checklist 可为当前可见引导上下文生成可复用的本地执行表。复制内容会组合当前规则和比较规则、选中的预设或课程路径、比较重点、引导备注、计时提示、设置检查项和反思问题，不会上传任何内容。
...
选择 Workshop 节奏后复制 Patterns from one spark 的 timing cues，可得到一份 45 分钟指南，其中 Explore、Compare 和 Reflect 的范围分别为 0-15、15-30 和 30-45 分钟。选择 Bell-ringer 时，会得到 12 分钟开场版本，范围为 0-4、4-8 和 8-12 分钟。
...
一次引导活动结束后，使用 Copy session summary 可把可见棋盘设置、当前比较结果、选中的上下文、标注和导出选项汇总成一段可复制文本。

把一次活动交给另一位引导师前，可以使用 Copy workshop checklist 生成简洁执行表，把保存的比较设置、选中的课程路径、计时提示、教师说明提示和反思问题连接起来。
...
Ruleloom Lab 使用 Vitest 编写面向行为的测试，覆盖规则解码、固定和环绕边界生成、确定性种子、精选图库元数据、图库筛选与应用行为、课程路径元数据与步骤应用、课程路径计时提示排程和纯文本提示表、课程路径节奏预设以及使用所选节奏的复制材料、课程路径包导入/导出校验、代数标注校验/导入/导出/排序、比较集合校验/存储/导入/导出/应用行为、会话摘要格式化、工作坊检查清单执行表格式化、投影模式标签/状态/类名、规则比较汇总、教师说明格式化与打印辅助函数、URL 查询辅助函数、预设解释、键盘快捷键、纯文本导出、SVG 导出、类 RLE 导出与导入、PNG 导出渲染与文件名、剪贴板/下载行为和渲染出的 HTML 结构。
...
Ruleloom Lab 现在进入维护节奏。下一阶段专注于小范围兼容性、可访问性、文案和本地材料维护，同时继续保持课堂流程本地优先。
